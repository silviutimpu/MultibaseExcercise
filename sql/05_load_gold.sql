-- MultiBase — 05 — Gold load procedure
-- Runs in: WH_Incidents
-- Called by: PL_Incidents_Load (Stored procedure activity)
--
-- Empties and refills; does not recreate the tables, so the Direct Lake model
-- stays bound. All in one transaction: if it fails halfway, gold is not left
-- half loaded.

CREATE OR ALTER PROCEDURE gold.LoadGold
AS
BEGIN
    BEGIN TRY
        BEGIN TRANSACTION;
        ...
        -- period label, from the first and last month WITH DATA in each year
        periods AS (
            SELECT
                YEAR(Month) AS Yr,
                CAST(CONCAT(
        ...
        SELECT
            ...
            YEAR(d.[Date]),
            MONTH(d.[Date]),
            -- CAST: DATENAME returns NVARCHAR, which Fabric does not accept
